...
public class TypeVisitor implements Visitor2{
	...
	Semant.Types.Type t2 = e.e2.accept(this);
	  ...
	  public Semant.Types.Type visit(ThreadDecl e){
		  ...
		  ot.beginScope();
		  Semant.Types.CLASS c = (Semant.Types.CLASS)get(Symbol.symbol(e.name));
		  Semant.Types.CLASS p = (Semant.Types.CLASS)get(Symbol.symbol(e.parent));
		  while(p != null){
				//TODO check for duplicates and bad overrides
			  for(Semant.Types.FIELD f : p.fields.fields){
				f.accept(this);
				c.fields.fields.add(f);
				}
			for (Semant.Types.FIELD f : p.fields.methods) {
				f.accept(this);
